    ...
    <div  className={ `w-full ${ active ? "bg-primary-400  z-50 shadow-xl font-lato fixed top-0 left-0  " :
      ...
      <div className={ `transition-all duration-500 flex justify-center items-center absolute overflow-x-hidden h-screen translate-x-0  w-3/4  top-[63px]  bg-primary-400/95 ${ isOpen ? "lg:hidden right-0 " : "-right-full" }` }>
        <div className="flex flex-col font-poppins items-center text-lg gap-10">
          <Link onClick={()=>setIsOpen(false)} to="home" activeClass="currently" smooth={ true } className="hover:font-bold hover:text-primary-200  transition-all duration-500 cursor-pointer">Home</Link>
          <Link onClick={()=>setIsOpen(false)} to="about"  activeClass="currently" smooth={ true }   className=" hover:font-bold hover:text-primary-200 transition-all duration-500  cursor-pointer">About Us</Link>
          <Link onClick={()=>setIsOpen(false)} to="services"  activeClass="currently" smooth={ true }   className=" hover:font-bold hover:text-primary-200 transition-all duration-500  cursor-pointer">Services</Link>
          <Link onClick={()=>setIsOpen(false)} to="contact"  smooth={ true }   className=" transition-all duration-500  bg-primary-200  hover:animate-wiggle cursor-pointer py-2 px-6 text-primary-400 text-lg  font-poppins rounded-br-2xl rounded-tl-2xl">Contact Us</Link>
        </div>
      </div>
    </div>
  )
